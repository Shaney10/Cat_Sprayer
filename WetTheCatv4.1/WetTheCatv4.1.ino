...
signed int rawWeight;             // Raw Value from ADC
signed int weightoffset = 0;      // Offset to bring pounds to zero
unsigned int SprayCount = 0;      // Spray Counter to wait for to turn on pump
unsigned int TotalCount = 0;      // Total Spray Counter
const int SprayReset = 3;         // Spray Counter to apply zero function if reached

const float kFactor = -0.00008;     // Multiplier to bring reading to pounds
...
const int highweight = 75;       // Weight value to not trigger event
const int triggercounter = 2;     // Spray counter to delay spray event
const int spraytime = 300;        // Sprya on time
...
//  Serial.begin(57600);  // Start Serial Stream
  ...
  rawWeight = (scale.read_average(20)* kFactor);  // Read average of 5 readings from the ADC
...
{
  rawWeight = (scale.read_average(5) * kFactor);             // Read average of 5 readings from the ADC
...
if (Weight >= lowweight && Weight <= highweight)  // Cat on board start counting
{
  digitalWrite (LED_BUILTIN, LOW);  // turn the LED off
  lcd.clear();                      // Clears the LCD screen
  lcd.setCursor(0, 0);              // Positions the cursor in the upper-left corner
  lcd.print("Hi_Kitty");
  counter = (counter + 1);          // counter to turn on sprayer
  lcd.setCursor(0, 1);
  lcd.print(counter);
  lcd.print(" Count to ");
  lcd.print(triggercounter);        // print trigger counter value to LCD
  delay(25);                         // Delay loop to check for weight again
} 
