...
const int SprayReset = 2;         //Spray Counter value to reset zero value

const int posoffset = 0;        // Value to offset by to keep value positive
const int poszero = 180;            // Weight value from lowweight value to zero
const int lowweight = 200;         // Weight value to trigger event
const int highweight = 1000;       // Weight value to not trigger event
...
// put your main code here, to run repeatedly:
{
  ...
  Weight = (WeightSensor * 2) - weightoffset + posoffset; // Add posoffset to keep value positive // Changed from 0.2 to 2
...
//  lcd.print("Pounds:");       // Print a message of "Pounds: "to the LCD.
  lcd.print(Weight);   // Print a weight value to the LCD.
//  lcd.print(Weight - posoffset);   // Print a weight value to the LCD.
...
{
  weightoffset = (Weight - posoffset + weightoffset);     // Zero weight value
...
if (Weight < posoffset or Weight >= lowweight - poszero + posoffset && Weight < lowweight + posoffset)
{
  zerocount = (zerocount + 1);
  delay(250);
}
else // of (Weight < posoffset)
{
  zerocount = (0);
}
  if (zerocount >= 5)
...
if (Weight >= lowweight + posoffset && Weight <= highweight + posoffset)  // Add posoffset to keep value positive
{
